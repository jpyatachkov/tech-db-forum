...
RUN echo "synchronous_commit = off" >> /etc/postgresql/$POSTGRES_VERSION/main/postgresql.conf &&\
    ...
    echo "effective_cache_size = 750MB" >> /etc/postgresql/$POSTGRES_VERSION/main/postgresql.conf &&\
    echo "work_mem = 64MB" >> /etc/postgresql/$POSTGRES_VERSION/main/postgresql.conf &&\
    echo "maintenance_work_mem = 256MB" >> /etc/postgresql/$POSTGRES_VERSION/main/postgresql.conf &&\
...
CMD service postgresql start &&  java -Xmx300M -Xms300M -jar $APP_ROOT/target/databases-0.0.1-SNAPSHOT.jar
